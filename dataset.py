...
dataset.py
----------
PyTorch Dataset for CUHK Face Sketch (CUFS) paired sketch-photo data.

Reads directly from the split folders created by 02_preprocessing_split.ipynb.

Pipeline per sample:
    disk PNG (uint8 [0,255])
        -> load with OpenCV
        -> PairedAugmentation (spatial + colour + normalize to [-1,1])
        -> torch tensor (C, H, W) float32

Expected folder structure:
    Sketch2Face/dataset/split/
    ├── train/
    │   ├── sketches/   <- preprocessed uint8 PNGs
    │   └── photos/     <- preprocessed uint8 PNGs
    └── test/
        ├── sketches/
        └── photos/
...
Usage:
    ...
    train_dataset = CUHKDataset('data/split/train', augment=True)
    test_dataset  = CUHKDataset('data/split/test',  augment=False)
