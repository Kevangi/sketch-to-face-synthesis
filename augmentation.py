...
Receives uint8 [0, 255] images from dataset.py.
Returns float32 [-1, 1] tensors ready for the model.

Responsibilities:
    - Spatial transforms  : flip, rotate, crop — applied to BOTH images identically
    - Colour transforms   : brightness, contrast, saturation, gamma — photo ONLY
    - Normalization       : [0, 255] -> [-1, 1] — applied to BOTH at the end

Usage:
    from augmentation import PairedAugmentation

    aug = PairedAugmentation(img_size=256, augment=True)   # train
    aug = PairedAugmentation(img_size=256, augment=False)  # test

    sketch_float, photo_float = aug(sketch_uint8, photo_uint8)
"""
